...
to detect-neighbors
  ...
  [
    set max-bisons-left-and-right 4
  ]
...
to select-prey [dt] ;; wolf procedure
  ...
  if handle-time <= 0
  [
    set nearest-prey min-one-of bisons with [calf? = true] [distance myself]
    let nearest-bison min-one-of bisons  with [calf? = false] [distance myself]
    ...
    [
      ...
      [
        if distance nearest-prey < lock-on-distance and attackable = true
        [
...
to-report attackable
  let is_attackable false
  ask nearest-prey
  [
     let nearest-bison min-one-of bisons  with [calf? = false] [distance myself]
     if distance nearest-bison > 2
  ...
  report is_attackable
end

to hunt [dt] ;; wolf procedure
  if nearest-prey != nobody
  [
    ifelse attackable = true
    [
